...
class AuthTokenAuthenticator extends Controller implements SimplePreAuthenticatorInterface, AuthenticationFailureHandlerInterface
{
    ...
    const TOKEN_VALIDITY_DURATION = 30*60;
    ...
    public function createToken(Request $request, $providerKey)
    {
        ...
        if (($request->getMethod() === "POST" && ($this->httpUtils->checkRequestPath($request, $targetUrl)||$this->httpUtils->checkRequestPath($request, $newUser)))||($request->getMethod() === "DELETE" && $this->httpUtils->checkRequestPath($request, $targetUrl))) {
            return;
        }
    ...
    {
        if (time() - $authToken->getCreatedAt()->getTimestamp() < self::TOKEN_VALIDITY_DURATION){
